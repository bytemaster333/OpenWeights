...
# OPENWEIGHTS_API_KEY fresh-minted via scripts/issue-test-key.sh
...
TOKEN="${OPENWEIGHTS_API_KEY:?OPENWEIGHTS_API_KEY required (mint via scripts/issue-test-key.sh)}"
...
# Route xet-core's CAS traffic at OpenWeights. HF_XET_DATA_CUSTOM_HEADERS carries
...
# -----------------------------------------------------------------------------
# [1/4] Pull the fixture from HF-native. No OpenWeights involvement yet; this
...
# Re-route to OpenWeights for the write + read legs.
export HF_XET_DATA_DEFAULT_CAS_ENDPOINT="$CAS"
export HF_XET_DATA_CUSTOM_HEADERS="Authorization=Bearer $TOKEN"

# -----------------------------------------------------------------------------
# [2/4] Upload to OpenWeights CAS via huggingface-cli upload. The CLI talks to
...
# OpenWeights CAS under test. The scratch repo name is suffixed with a timestamp
# so successive nightly runs don't collide.
# -----------------------------------------------------------------------------
echo "[2/4] upload to OpenWeights via $HF_CLI"
TEST_REPO="openweights-ci/roundtrip-$(date +%s)"
# `|| true` — hf.co may reject the scratch repo if openweights-ci doesn't exist;
# the round-trip itself does not depend on the upload succeeding at hf.co,
# only on xet-core xorb PUTs reaching openweights-cas. We verify via the xorbs
...
# -----------------------------------------------------------------------------
# [3/4] Download the same pinned revision, routing xet-core through OpenWeights.
# -----------------------------------------------------------------------------
echo "[3/4] $HF_CLI download via OpenWeights CAS endpoint"
"$HF_CLI" download "$REPO" \
  --revision "$REV" \
