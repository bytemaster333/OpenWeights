...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'siahub_gw') THEN
        RAISE EXCEPTION
            'siahub_gw role does not exist; create it before applying 0005. '
            'Either rebuild the postgres volume (triggers ops/indexd-postgres-init.sql) '
            'or run: CREATE ROLE siahub_gw LOGIN PASSWORD ''<SIAHUB_GW_POSTGRES_PASSWORD>'';';
    END IF;
END
$$;
