...
/// ```ignore
/// use openweights_cas_core::auth::AuthScoped;
/// use openweights_cas_core::scopes::SCOPE_UPLOAD;
...
pub trait AuthStateRef: Send + Sync + Clone + 'static {
    ...
    /// HS256 key used to sign/verify OpenWeights-issued Xet JWTs
    /// (`XET_JWT_SIGNING_KEY`). Returned owned so the extractor future stays
    /// `Send + 'static`. Empty when the HF-compat mint path is unconfigured.
...
async fn authenticate_xet_jwt<const S: u8>(
    ...
    let (user_id, api_key_id) = if claims.iss == xet_jwt::OPENWEIGHTS_ISSUER {
        // SECURITY: this token claims to be one WE minted, and it grants scope
        // straight from its `access` claim. We hold the HMAC key, so verify the
        // signature — otherwise a forged `{iss: OPENWEIGHTS_ISSUER, access:
        // WRITE, userId: <any existing id>}` would be trusted for write auth.
        // (HF-issued tokens, handled in the else branch, remain unverifiable by
        // necessity — HF exposes no JWKS — per the xet_jwt module docs.)
        if let Err(e) = xet_jwt::verify_openweights_signature(token, xet_jwt_signing_key) {
            tracing::warn!(err = %e, "openweights-issued xet jwt signature invalid — rejecting");
            return Err(AppError::Unauthenticated);
        }
        let uid = claims
            .user_id
        ...
        let key_id = match key_row {
            Some((id,)) => id,
            None => {
                xet_jwt::upsert_synthetic_api_key(pool, uid, &format!("openweights:{uid}"))
                    .await
                    .map_err(|e| {
...
/// OpenWeights-issued API keys are opaque url-safe random strings with no dots;
    ...
    St: AuthStateRef,
{
    ...
    fn from_request_parts(
        ...
        let jwt_signing_key = state.xet_jwt_signing_key();
        ...
        // through openweights-hf-proxy. hf_xet actually ships the HF JWT as the
        ...
        async move {
                ...
                .and_then(|s| s.strip_prefix("Bearer "))
            {
                ...
                None => {
                    if let Some(token) = xet_jwt_header.as_deref() {
                        return authenticate_xet_jwt::<S>(&pool, &jwt_signing_key, token).await;
                    }
                    return Err(AppError::Unauthenticated);
                }
            };
            ...
            if looks_like_jwt(plaintext) {
                return authenticate_xet_jwt::<S>(&pool, &jwt_signing_key, plaintext).await;
            }
