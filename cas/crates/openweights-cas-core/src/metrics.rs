...
use openweights_cas_storage::reconciler::ReconcilerMetrics;

/// Prometheus handle set for openweights-cas.
...
impl Metrics {
    ...
    pub fn new() -> Self {
        ...
        let xorb_orphaned_total = IntCounter::with_opts(Opts::new(
            "openweights_cas_xorb_orphaned_total",
        ...
        let shard_orphaned_total = IntCounter::with_opts(Opts::new(
            "openweights_cas_shard_orphaned_total",
        ...
        let shard_version_rejected_total = IntCounterVec::new(
            Opts::new(
                "openweights_cas_shard_version_rejected_total",
        ...
        let reconciler_sweeps_total = IntCounter::with_opts(Opts::new(
            "openweights_cas_reconciler_sweeps_total",
        ...
        let reconciler_failures_total = IntCounter::with_opts(Opts::new(
            "openweights_cas_reconciler_failures_total",
        ...
        let sia_upload_duration_seconds = Histogram::with_opts(
            HistogramOpts::new(
                "openweights_cas_sia_upload_duration_seconds",
        ...
        let sia_pin_duration_seconds = Histogram::with_opts(
            HistogramOpts::new(
                "openweights_cas_sia_pin_duration_seconds",
...
/// this is the cross-crate adapter that keeps `openweights-cas-storage` from
/// importing `openweights-cas-core` (which would form a dependency cycle, since
/// `openweights-cas-core` depends on `openweights-cas-storage` for `SiaAdapter`).
...
mod tests {
    ...
    fn metrics_registry_includes_phase_2_series() {
        ...
        let s = m.encode();
        assert!(
            s.contains("openweights_cas_xorb_orphaned_total"),
            "xorb orphaned series present"
        );
        assert!(
            s.contains("openweights_cas_shard_orphaned_total"),
            "shard orphaned series present"
        );
        assert!(
            s.contains("openweights_cas_shard_version_rejected_total"),
            "shard version rejected series present"
        );
        assert!(
            s.contains("openweights_cas_reconciler_sweeps_total"),
            "reconciler sweeps series present"
        );
        assert!(
            s.contains("openweights_cas_reconciler_failures_total"),
            "reconciler failures series present"
        );
        assert!(
            s.contains("openweights_cas_sia_upload_duration_seconds"),
            "sia upload duration histogram present"
        );
        assert!(
            s.contains("openweights_cas_sia_pin_duration_seconds"),
