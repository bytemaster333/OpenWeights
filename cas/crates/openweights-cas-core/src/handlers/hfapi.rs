...
//!   commit): `Authorization: Bearer <OpenWeights API key>`. We reuse the
...
/// `(path, xet_hash, lfs_oid, size)` — rows staged by the commit handler.
type CommitFileRow = (String, Option<Vec<u8>>, Option<Vec<u8>>, i64);
/// `(path, size, xet_hash, lfs_oid)` — repo file listing rows.
type ModelFileRow = (String, i64, Option<Vec<u8>>, Option<Vec<u8>>);
/// Four nullable aggregate counters returned by the repo stats query.
type RepoCountsRow = (Option<i64>, Option<i64>, Option<i64>, Option<i64>);
...
pub trait HfApiState: AuthStateRef {
    // The HS256 Xet-JWT signing key lives on `AuthStateRef::xet_jwt_signing_key`
    // (shared with the auth extractor, which verifies OpenWeights-issued tokens
    // with it). Empty slice → HF-compat mint is disabled and the handler 503s.
...
pub async fn xet_write_token<S: HfApiState>(
    ...
    xet_token(&st, &ctx, &owner, &repo, OpenWeightsAccess::Write, WRITE_TOKEN_TTL_SECS).await
}
...
pub async fn xet_read_token<S: HfApiState>(
    ...
    xet_token(&st, &ctx, &owner, &repo, OpenWeightsAccess::Read, READ_TOKEN_TTL_SECS).await
}
...
async fn optional_api_key_id<S: HfApiState>(
    ...
    // only opaque OpenWeights API keys (sha256 → api_keys row) count toward
...
pub async fn commit<S: HfApiState>(
    ...
    let pool = st.pool();
    let mut to_insert: Vec<CommitFileRow> =
...
async fn build_model_info<S: HfApiState>(
    ...
    let files: Vec<ModelFileRow> = sqlx::query_as(
...
async fn fetch_download_counters(
    ...
    let row: Result<RepoCountsRow, _> =
        sqlx::query_as(
            "SELECT \
...
async fn resolve_lookup<S: HfApiState>(
    ...
    let file_row: Option<ResolveFileRow> = if revision == "main" {
        sqlx::query_as(
...
pub async fn xet_file_serve<S: HfApiState>(
    ...
    use openweights_cas_proto::xorb_object::deserialize_chunk_to_writer;
    use std::io::Cursor;
