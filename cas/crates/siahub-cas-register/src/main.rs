//! siahub-cas-register — one-shot CLI that registers the SiaHub CAS as an
...
//! Flow:
//! 1. Read SIAHUB_APP_ID (hex) + SIAHUB_RECOVERY_PHRASE + SIAHUB_INDEXER_URL from env.
...
async fn main() -> Result<()> {
    let indexer_url =
        env::var("SIAHUB_INDEXER_URL").context("SIAHUB_INDEXER_URL not set")?;
    let app_id_hex = env::var("SIAHUB_APP_ID").context("SIAHUB_APP_ID not set")?;
    let mnemonic = env::var("SIAHUB_RECOVERY_PHRASE")
        .context("SIAHUB_RECOVERY_PHRASE not set")?;
    ...
    let meta = AppMetadata {
        id: app_id,
        name: "SiaHub CAS",
        description: "SiaHub content-addressed storage service",
        service_url: "https://siahub.app",
        logo_url: None,
        callback_url: None,
    };
    ...
    eprintln!("     -d '{{\"description\":\"siahub-cas\",\"quota\":\"default\"}}' \\");
    ...
    eprintln!();
    eprintln!("SUCCESS. Append to .env (or overwrite SIAHUB_APP_KEY):");
    println!("SIAHUB_APP_KEY={b64}");
