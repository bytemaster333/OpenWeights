...
async fn run(cfg: Config) -> anyhow::Result<()> {
    let pool = openweights_cas_db::build_pool(&cfg.database_url, cfg.pg_max_connections)
        .await
    ...
    openweights_cas_db::run_migrations(&pool)
        .await
    ...
    let key_cache = Arc::new(openweights_cas_core::auth::KeyCache::new(10_000));
    let rate_limit_defaults = openweights_cas_core::rate_limit::RateLimitDefaults::default();
    ...
    let sia: Arc<dyn openweights_cas_storage::SiaAdapter> = build_sia_adapter(&cfg)
        .await
    ...
    let signer = openweights_cas_core::signed_url::UrlSigner::new(
        &cfg.gateway_url_signing_key,
        // An empty GATEWAY_URL_SIGNING_KEY_PREV (compose passes `${..:-}` = "")
        // means "no previous key", not a zero-length key. Filter it to None so
        // the signer doesn't try to base64-decode "" into a 32-byte HMAC key.
        cfg.gateway_url_signing_key_prev
            .as_deref()
            .filter(|s| !s.is_empty()),
        gateway_base,
    ...
    let metrics = Arc::new(openweights_cas_core::metrics::Metrics::new());
    ...
    let http_client = Arc::new(
        reqwest::Client::builder()
            .user_agent("openweights-cas/1.0")
            .build()
    ...
    tokio::spawn(async move {
        openweights_cas_storage::reconciler::run_reconciler(
            reconciler_pool,
            reconciler_sia,
            reconciler_metrics,
        )
    ...
    tracing::info!("openweights-cas ready");

    let bind_addr = cfg.bind_addr.clone();
    let app = router::build(state);
    let listener = tokio::net::TcpListener::bind(&bind_addr).await?;
    tracing::info!(addr = %bind_addr, "openweights-cas listening");
...
/// 2. Otherwise, decode OPENWEIGHTS_APP_ID (hex) + OPENWEIGHTS_APP_KEY (base64 32
///    bytes) and run the A1-probe builder sequence.
async fn build_sia_adapter(
    cfg: &Config,
) -> anyhow::Result<Arc<dyn openweights_cas_storage::SiaAdapter>> {
    #[cfg(feature = "sia-mock")]
    {
        if std::env::var("OPENWEIGHTS_SIA_MOCK").as_deref() == Ok("true") {
            tracing::warn!(
                "OPENWEIGHTS_SIA_MOCK=true — wiring in-memory MockSiaAdapter. \
                 DEV ONLY: bytes are NOT durable on Sia."
            );
            return Ok(Arc::new(openweights_cas_storage::mock::MockSiaAdapter::new()));
        }
    }

    // Decode the base64 app key → 32 bytes.
    let key_bytes_vec = B64
        .decode(cfg.openweights_app_key.as_bytes())
        .context("OPENWEIGHTS_APP_KEY is not valid base64")?;
    let app_key_bytes: [u8; 32] = key_bytes_vec
        .as_slice()
        .try_into()
        .map_err(|_| anyhow!("OPENWEIGHTS_APP_KEY must decode to exactly 32 bytes"))?;

    // Decode the hex app_id (64 chars).
    if cfg.openweights_app_id.len() != 64 {
        return Err(anyhow!("OPENWEIGHTS_APP_ID must be 64 hex chars"));
    }
    let mut id_bytes = [0u8; 32];
    for (i, slot) in id_bytes.iter_mut().enumerate() {
        *slot = u8::from_str_radix(&cfg.openweights_app_id[2 * i..2 * i + 2], 16)
    ...
    let meta = sia_storage::AppMetadata {
        id: app_id,
        name: openweights_cas_storage::sia::DEFAULT_APP_META_NAME,
        description: openweights_cas_storage::sia::DEFAULT_APP_META_DESC,
        service_url: openweights_cas_storage::sia::DEFAULT_APP_META_URL,
        logo_url: None,
        callback_url: None,
    };
    ...
    let sia_cfg = openweights_cas_storage::sia::SiaAdapterConfig {
        indexd_url: cfg.indexd_url.clone(),
        app_key_bytes,
        app_meta: meta,
        redundancy: openweights_cas_storage::sia::RedundancyConfig {
            data_shards,
            parity_shards,
        },
    };

    let adapter = openweights_cas_storage::sia::RustSdkAdapter::connect(sia_cfg)
        .await
