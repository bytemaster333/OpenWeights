//! Postgres pool + migrations wiring for openweights-cas.
...
pub async fn run_migrations(pool: &sqlx::PgPool) -> Result<(), sqlx::migrate::MigrateError> {
    // NOTE: the Dockerfile MUST `COPY cas/migrations` before the release build,
    // otherwise this macro embeds an empty dir and no schema is created.
