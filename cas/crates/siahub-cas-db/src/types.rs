...
/// Encoded as an integer at the extractor level (Upload=0, Download=1,
...
/// `shards` tables. Reconciler drives state transitions:
...
/// item 1 folds in `Reconstruction`. `XorbServe` + `DedupQuery` are
/// reserved slots consumed by gateway (OQ-J) and a future non-stub
/// of respectively; code must not emit them.
