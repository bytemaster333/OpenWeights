...
//! authenticate to SiaHub's CAS after the `siahub-hf-proxy` rewrites
...
//! * Exchange the HF JWT for a SiaHub-issued JWT at the proxy and sign
...
/// that came through siahub-hf-proxy.
/// * `https://hf.siahub.app` — tokens minted by THIS server when SiaHub
...
const EXPECTED_ISSUERS: &[&str] = &["https://huggingface.co", "https://hf.siahub.app"];

/// The issuer siahub-cas writes into tokens it mints itself.
pub const SIAHUB_ISSUER: &str = "https://hf.siahub.app";
...
/// SiaHub API key, we conjure one sentinel row per user so the FK holds.
/// The row:
...
/// Mint a SiaHub-issued Xet JWT for an HF-compat upload/download session.
...
pub fn mint_siahub_token(
    ...
    let access_str = match access {
        SiaHubAccess::Write => "WRITE",
        SiaHubAccess::Read => "READ",
    };
...
/// Scope marker for `mint_siahub_token`. Maps 1:1 to HF's `access` claim
/// string; `Write` grants upload+download, `Read` grants download only.
#[derive(Debug, Clone, Copy)]
pub enum SiaHubAccess {
    Write,
    Read,
}
...
mod tests {
    ...
    #[test]
    fn siahub_minted_token_round_trips_through_decode_and_validate() {
        ...
        assert_eq!(claims.access, "WRITE");
        assert_eq!(claims.iss, SIAHUB_ISSUER);
        assert_eq!(
            claims.scopes(),
            vec![ApiKeyScope::Upload, ApiKeyScope::Download]
        );
