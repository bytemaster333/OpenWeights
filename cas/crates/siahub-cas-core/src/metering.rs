...
//!
//! Plan 02-09 / OPS-05 / D-17. Every xorb_upload, shard_upload, and
...
//! for every metering call-site — Phase 5 OQ-K (bytes/min pivot if the
//! req/min bucket misbehaves) rewrites these three functions only, without
//! touching handlers.
//!
//! Error semantics (D-17 + plan Task 1):
...
//!     All Phase 2 handlers treat post-commit metering as best-effort: they
...
/// Record a successful xorb upload. Called by Plan 02-04 on both the
...
/// Record a successful shard upload (post-commit variant — Plan 02-05 dedup
...
/// (D-17). Failure aborts the tx — acceptable, documented in module docs.
...
/// reconstruction is a metadata read with no payload. Phase 3 gateway will
