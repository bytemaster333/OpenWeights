...
///
/// Capacity is locked to 10 000 at construction (D-20). Cache is NOT shared
...
/// Plan 02-02 (W2) migrations are in flight in a sibling worktree. Once W2
    ...
    fn from_request_parts(
        ...
        async move {
            ...
            let plaintext = auth_header
                ...
                .ok_or(AppError::Unauthenticated)?; // 401
            ...
            let key = if let Some(cached) = cache.get(&hash) {
                ...
                match fetch_active_key_by_hash(&pool, &hash).await? {
                    ...
                    None => {
                        // Unknown hash = 401 (NEVER 403 — T-02-03-08 + PROTO-09).
