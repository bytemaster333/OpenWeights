...
use siahub_cas_proto::metadata_shard::shard_format::test_routines::{
...
#[test]
fn p19_footer_version_not_one_is_rejected() {
    let (mut bytes, _xorb) = build_valid_shard_bytes();
    // Footer layout: last 200 bytes. First u64 (offset 0 within footer) is
    // `version`. Patch the LE word to 2.
    let footer_start = bytes.len() - 200;
    bytes[footer_start] = 2;
    for b in &mut bytes[footer_start + 1..footer_start + 8] {
        *b = 0;
    }

    let err = shard_parse::parse_and_validate(&bytes).expect_err("must reject footer v2");
    match err {
        ShardParseError::FooterVersion(v) => assert_eq!(v, 2),
        other => panic!("expected FooterVersion(2), got {other:?}"),
    }
...
#[test]
fn upload_shard_response_serializes_with_pascal_case_labels() {
    ...
    assert!(
        json.contains("\"SyncPerformed\""),
        "wire label must be exact: {json}"
    );
