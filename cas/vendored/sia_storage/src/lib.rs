...
impl UploadOptions {
    ...
    pub fn validate(&self) -> Result<(), UploadError> {
        ...
        // satisfy the 99.99% recovery threshold below. We dial this up to
        // 0.90 so a 2-of-6 erasure scheme (3x redundancy) clears the
        // 99.99% recovery threshold with the actual host pool we have.
        // 0.90 is conservative vs measured testnet host SLA (~95%+ on
        // hostd v5.x). This is a deployment-time tuning knob, not a
        // correctness change: the binomial CDF math below is identical,
        // only the per-shard p moves. Math: with p=0.90, n=6, k=2:
        //   P(X<2) = (0.1)^6 + 6·0.9·(0.1)^5 = 5.5e-5
        //   P(X>=2) = 0.99994 ≥ 0.9999 ✓
        const RECOVERY_PROBABILITY: f64 = 0.90;
