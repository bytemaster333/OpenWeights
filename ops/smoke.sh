...
# OPENWEIGHTS_PUBLIC_READ_KEY read-scoped API key (mint one via the console)
# OPENWEIGHTS_FIXTURE_FILE_ID file-id from ops/preload-fixture.sh receipt
...
# Source.env if present — makes OPENWEIGHTS_PUBLIC_READ_KEY / OPENWEIGHTS_FIXTURE_FILE_ID
...
echo "=== OpenWeights hosted-demo smoke against ${DOMAIN} ==="
...
if [[ -n "${OPENWEIGHTS_FIXTURE_FILE_ID:-}" && -n "${OPENWEIGHTS_PUBLIC_READ_KEY:-}" ]]; then
  # Authenticated call: expect a 200 with a fetch_info shape.
  RECON="$(
    curl -fsS --max-time 15 \
      -H "Authorization: Bearer ${OPENWEIGHTS_PUBLIC_READ_KEY}" \
      "${CAS}/v1/reconstructions/${OPENWEIGHTS_FIXTURE_FILE_ID}" \
    || echo ""
  )"
...
# ─── 4. HF download via OpenWeights endpoint (optional; core-value check) ──────
...
if [[ -z "$HF_BIN" ]]; then
...
elif [[ -z "${OPENWEIGHTS_PUBLIC_READ_KEY:-}" ]]; then
  echo "[WARN] OPENWEIGHTS_PUBLIC_READ_KEY unset; skipping download check (not a fail)"
else
  TMP="$(mktemp -d)"
  trap 'rm -rf "$TMP"' EXIT
  # shellcheck disable=SC1091
  ...
  if HF_XET_DATA_DEFAULT_CAS_ENDPOINT="${CAS}" \
     HF_XET_DATA_CUSTOM_HEADERS="Authorization=Bearer ${OPENWEIGHTS_PUBLIC_READ_KEY}" \
     "$HF_BIN" download "$HF_FIXTURE_REPO" "${HF_ARGS[@]}" >/dev/null 2>&1; then
    pass "hf download via OpenWeights endpoint succeeded"
  else
    fail "hf download via OpenWeights endpoint failed"
  fi
fi
...
if [[ -x tests/hf-roundtrip/verify-range-integrity.sh ]]; then
  ...
  if GATEWAY_VIA_CADDY_URL="${CAS}/gateway" \
     ...
     OPENWEIGHTS_API_KEY="${OPENWEIGHTS_PUBLIC_READ_KEY:-}" \
     XORB_HASH="${OPENWEIGHTS_FIXTURE_XORB_HASH:-}" \
     SIGNED_URL_PATH="${OPENWEIGHTS_FIXTURE_SIGNED_URL_PATH:-}" \
    ...
    RC=$?
    if [[ "$RC" -eq 2 ]]; then
      echo "[WARN] range-integrity script needs OPENWEIGHTS_FIXTURE_XORB_HASH or SIGNED_URL_PATH; skipping"
    else
