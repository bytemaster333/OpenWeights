...
#
# D-58: one pinned-revision fixture used by EVERY harness that touches HF:
#   - bench/run.sh                   (Phase 5 plan 05-03 — this directory's harness)
#   - tests/hf-roundtrip/run.sh      (Phase 5 plan 05-02 — byte-identical multi-GB round-trip)
#   - ops/smoke.sh                   (Phase 5 plan 05-04 — hosted-demo post-deploy smoke)
#
# Probe 1 resolution (see .planning/phases/05-ship-v1/05-03-benchmarks-report-PLAN.md §Task 1):
...
#     (c) 78 MB total — trivially fits gateway LRU cache (20 GB default, GATE-06)
...
# and re-run both PROTO-11 (05-02) and BENCH-REPORT (05-03).
...
# routes xet-core through HF's own S3+CloudFront CAS. This is the BENCH-04
...
# Bench trial count — D-59: 3 trials, median only. No P10/P90/stddev.
