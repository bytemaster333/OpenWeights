...
# "rows": [
# { "scenario": "cold-cache", "openweights_mbps": <n|null>, "hf_baseline_mbps": <n|null> },
# { "scenario": "warm-cache",... },
# { "scenario": "upload",... }
# ] }
...
cat > "$OUT_JSON" <<EOF
{
  ...
  "rows": [
    { "scenario": "cold-cache", "openweights_mbps": $(jnum "$SH_COLD_MBPS"), "hf_baseline_mbps": $(jnum "$HF_COLD_MBPS") },
    { "scenario": "warm-cache", "openweights_mbps": $(jnum "$SH_WARM_MBPS"), "hf_baseline_mbps": $(jnum "$HF_WARM_MBPS") },
    { "scenario": "upload",     "openweights_mbps": $(jnum "$SH_UP_MBPS"),   "hf_baseline_mbps": null }
  ],
  "fixture": {
    "repo": "$REPO",
    "revision": "$REV",
  ...
  "raw": {
    "openweights_cold_seconds": $(jnum "$SH_COLD"),
    "openweights_warm_seconds": $(jnum "$SH_WARM"),
    "openweights_upload_seconds": $(jnum "$SH_UP"),
...
- **Upload comparison** — \`huggingface-cli upload\` talks to \`hf.co\` for the metadata CAS-commit regardless of which stack is serving xorbs. The OpenWeights-vs-HF-native distinction only matters at the xorb PUT layer; the non-xorb metadata path is identical. We therefore publish OpenWeights upload times only and explicitly omit a misleading "HF-native upload" column.
- **Hardware / network:** measurements were taken on the owner's server; spec + network conditions are documented in the repo README §Prerequisites. Reproducibility of the numbers depends on that environment — re-running \`make benchmark\` on a different host will produce different absolute numbers but the OpenWeights-vs-HF-native ratio should stay within an order of magnitude.
...
\`\`\`
# Measure only OpenWeights (skip HF-native).
make benchmark STACK=openweights

# Measure OpenWeights + HF-native baseline for comparison (default).
make benchmark
