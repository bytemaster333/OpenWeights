...
//     here; plan 03-04 replaces that branch with `multipart/byteranges`.
//
...
//     (gotcha #8 — xet-core's refresh path is keyed exclusively to 403;
...
//     path; plan 03-05 replaces `getXorbSource` with a cache-aware variant
...
//       - many ranges   → 501 (03-04 owns)
//     Streaming via `io.Copy(w, io.NewSectionReader(...))` — whole-object
//     buffering in RAM would violate GATE-08.
//  7. Client-disconnect: everywhere we touch I/O we use `r.Context()` (GATE-10).
...
//     janitor cleans), not worth wrapping in recover. 03-05 replaces this
//     entirely.
package main
...
type Handlers struct {
	...
	Sia      SiaDownloader
	Meter    UsageWriter
	// Cache is the whole-xorb disk LRU wired in by plan 03-05. Nil-safe for
	// unit tests that only exercise verifier / DB branches; nil triggers the
	// old SDK-direct temp-file path (Wave 3a behavior).
	Cache *Cache
	// Miss coalesces concurrent cold-miss fetches for the same hash (D-30 /
	// GATE-09). Nil-safe — nil falls back to un-coalesced per-request Sia
...
//
// OPS-02 / GATE-11: emits exactly one structured JSON log line per request
...
func (h *Handlers) ServeXorb(w http.ResponseWriter, r *http.Request) {
	...
	//    Wave 3b (plan 03-05) replaces this with a cache-aware variant.
	if h.Sia == nil {
	...
	// 6. Write the response. X-Cache + X-Sia-Fetch-Ms per GATE-11 — this
	//    handler always emits MISS until 03-05 lights up the cache HIT path.
	cacheLabel := "MISS"
	if src.CacheHit {
		cacheLabel = "HIT"
	}
	...
	case len(ranges) == 1:
		...
		// Note: if Copy fails mid-flight we cannot reset the status — headers
		// and WriteHeader(206) have already been sent. Log from 03-06 handles
		// the structured line.
	...
	default:
		...
		//
		// GOTCHA #3 is GUARDED here: we call `writeMultipartRanges` (which
		...
		// reset the status. Headers + 206 have been flushed. 03-06 structured
...
// JSON emission mandated by OPS-02 / ROADMAP §Phase 3 SC-6. Fields match
...
//
// Fields match OPS-02 / ROADMAP §Phase 3 SC-6 verbatim.
...
// downstream appenders are ignored (the Caddy reverse proxy in Phase 6
...
// code the handler contract requires. Gotcha #8 is the single load-bearing
...
// DB error messages that must not leak to clients) — the OPS-02 structured
...
// xorbSource is the seam shared with plan 03-05. Fields:
//
...
// Wave 3b (plan 03-05) variant — cache-first with singleflight-coalesced
// cold-miss path:
//
...
//     request latency for popular xorbs (GATE-06: <50ms p50 on warm cache).
//
//  2. COLD PATH — cache MISS. `MissCoalescer.Do(hash, fetchFn)` ensures
//     only one Sia fetch runs per xorb hash per concurrent cohort (D-30 /
//     GATE-09). fetchFn re-checks the cache (a concurrent leader may have
...
//     with hash-verify-on-write (GATE-07 / P11).
//
...
// via the inner SDK. A client TCP RST propagates within 1s (GATE-10). If
...
//
// Kept verbatim from pre-03-05 for test-suite compatibility.
