...
//
// SCOPE: this file owns the single-range half. Plan 03-04 extends
...
// slice — so 03-04 only has to add a new writer, not touch the parser again.
//
// The load-bearing nuance addressed here is the HTTP-end-inclusive vs
// SDK-offset+length boundary (ROADMAP §Phase 3 SC-1). An HTTP byte-range
...
//
// A5 PROBE VERDICT (04-2026, see .planning/phases/03-siahub-gateway-core/
...
// log grep per ROADMAP §Phase 3 SC-2 and per RECEIVED §C invariant 10
...
//
// NOTE: the `[2]uint64` form used by the signed-URL verifier (`VerifyOk.Range`)
...
//
// Multi-spec (comma-separated) is parsed and returned verbatim so plan 03-04
...
//
// NOTE: the signed-URL verifier's `r=S-E` query param carries an independent
...
// `io.NewSectionReader` → `io.Copy`. No whole-object buffering — GATE-08 is
...
// THE single most integrity-critical function in Phase 3 — notes.md gotcha #3
...
// exactly the class of off-by-CRLF bug that gotcha #3 warns about. The stdlib
...
// Closing: "--" + boundary + "--\r\n"
//
// GATE-08 (no whole-object buffering): each part body is streamed via
...
// `bytes=0-99,50-149` they get two parts, with byte range 50-99 appearing in
