...
/**
 * API-key scopes understood by CAS `/admin/keys` (see 04-01 Task 4).
 *
 ...
 * creation — `GET /admin/keys` returns the masked prefix only. See D-45:
 ...
 *
 * Load-bearing invariant (D-45, 04-05 plan Task 1): this shape MUST NOT
 ...
 *
 * `staleTime: 10_000` matches D-47's stats cadence — the keys list is
 * intentionally not hot-polled. Server-side, last-used-at refresh is
 * capped at 5s debounce (04-01 task 8), so a 10s stale window never shows
 ...
 *
 * KEYS-04 SLO (5s): enforced server-side (Redis cache invalidation + auth
