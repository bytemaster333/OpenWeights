...
function countryFlag(code: string | null): string {
  ...
  return String.fromCodePoint(cc.charCodeAt(0) - 0x41 + 0x1f1e6, cc.charCodeAt(1) - 0x41 + 0x1f1e6)
}
...
export function MapPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return hosts.filter(
      (h) =>
        h.public_key.toLowerCase().includes(q) || (h.country_code?.toLowerCase() ?? "").includes(q),
    )
  }, [hosts, query])
  ...
  const totalContracts = (hosts ?? []).reduce((n, h) => n + (h.contract_count ?? 0), 0)
    ...
    <main className="mx-auto max-w-6xl space-y-6 px-6 py-8">
      <header>
        <div className="flex items-center gap-3">
          ...
          <h1 className="font-heading text-2xl font-semibold tracking-tight">Storage providers</h1>
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <SummaryCard
          ...
          hint="contractable"
        />
        <SummaryCard label="Countries" value={countries.length.toString()} hint="distinct" />
        <SummaryCard
        ...
        <aside className="flex h-[560px] flex-col rounded-lg border bg-muted/10">
          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {!isPending && filtered.length === 0 && (
              <div className="p-6 text-center text-xs text-muted-foreground">No hosts.</div>
            )}
            {!isPending &&
              filtered.map((h) => (
                <button
